...
import 'package:nzoapp/utils/Constantes.dart';
...
class HttpResponse {
  ...
  HttpResponse(
      {this.data, required this.success, this.errorMsg, this.isException});
}
...
Future<HttpResponse> postData(String api_url, Map data, {String? token}) async {
  try {
   ...
   /* if(!kReleaseMode){
    ...
    return HttpResponse(success: st, data: msg);
  } catch (e, trace) {
    print(e.toString());
    print(trace.toString());
    return HttpResponse(
        success: false, errorMsg: "Erreur inattendue", isException: true);
  }
}
